<template>
  <div class="flex flex-col h-screen">
    ...
    >
      <div>
        ...
        <p class="text-[13px] text-koyori-ink-3 m-0 mt-0.5">
          Powered by every transcript in your library [Dummy]
        </p>
      </div>
    ...
    <div
      ...
      class="flex-1 overflow-y-auto px-8 py-4 max-w-[900px] w-full"
    >
      <div class="flex flex-col gap-5">
        <div v-for="(msg, i) in displayMessages" :key="i">
          ...
          <div v-if="msg.role === 'user'" class="flex justify-end">
            <div
              class="max-w-[580px] bg-koyori-pink-100 rounded-[16px_16px_4px_16px] px-4 py-3 text-[14px] text-koyori-ink leading-relaxed border border-koyori-pink-200"
            >
              {{ msg.text }}
            </div>
          </div>
          ...
          <div v-else class="flex gap-3 items-start">
            <!-- Avatar -->
            <div
              class="w-8 h-8 rounded-full shrink-0 bg-gradient-to-br from-koyori-pink-400 to-koyori-emerald-500 flex items-center justify-center"
            >
              <i class="pi pi-sparkles text-[14px] text-white"></i>
            </div>
            <div class="flex-1 min-w-0">
              <div class="text-[14px] text-koyori-ink leading-[1.65] mb-2.5">
                {{ msg.text }}
              </div>
              <!-- Citations -->
              <div
                v-if="msg.cites && msg.cites.length > 0"
              ...
              >
                ...
                >
                  <i
                    class="pi pi-play-circle text-[13px] text-koyori-pink-400"
                  ></i>
                  <div>
                    <div
                      class="text-[12px] font-semibold text-koyori-ink max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap"
                    >
                      {{ cite.stream }}
                    </div>
                    <div
                      class="text-[11px] text-koyori-ink-4 font-[JetBrains_Mono]"
                    >
                      {{ cite.date }} · {{ cite.t }}
                    </div>
                  </div>
        ...
        <div v-if="isThinking" class="flex gap-3 items-start">
          ...
          <div
            class="flex items-center gap-1 px-4 py-3.5 bg-white border border-koyori-border rounded-[16px_16px_16px_4px]"
          >
            <div
              v-for="n in 3"
              :key="n"
              ...
              :style="{
                animation: `bounce 1.4s ease-in-out ${(n - 1) * 0.16}s infinite`,
              }"
            ></div>
          </div>
        </div>
    ...
    <div class="shrink-0 px-8 pb-6 pt-4 max-w-[900px] w-full bg-transparent">
      ...
      <div
        v-if="!inputText.trim() && displayMessages.length < 4"
      ...
      <div class="text-[11px] text-koyori-ink-4 mt-1.5 text-center">
