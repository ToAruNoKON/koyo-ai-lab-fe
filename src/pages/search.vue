<template>
  <div class="flex flex-col min-h-full">
    ...
    <div class="flex items-center justify-between px-8 py-4 border-b border-koyori-border bg-white sticky top-0 z-10 shrink-0">
      <div>
        <h2 class="text-[18px] font-bold text-koyori-ink m-0 leading-tight">Search</h2>
        <p class="text-[13px] text-koyori-ink-3 m-0 mt-0.5">Across 89 streams · 24h 12m of audio [Dummy]</p>
      </div>
      <div class="flex gap-2">
        <NuxtLink to="/chat">
          <PGhostButton label="Ask the corpus →" size="small" icon="pi pi-sparkles" />
        </NuxtLink>
      </div>
    </div>
    ...
    <div class="flex-1 p-6 px-8 max-w-[1100px] w-full">
      ...
      <div class="relative mb-5">
        <i class="pi pi-search absolute left-4 top-1/2 -translate-y-1/2 text-koyori-ink-3 text-[16px] pointer-events-none"></i>
        <input
          v-model="searchQuery"
          type="text"
      ...
      <div class="flex items-center gap-3 mb-6 flex-wrap">
        <!-- Topic filter -->
        <div class="flex bg-koyori-surface-2 rounded-full p-0.5 gap-0.5 shrink-0">
          <button
            ...
            :class="activeTopic === topic ? '!bg-white !text-koyori-ink !font-semibold shadow-koyori-sm' : ''"
        ...
        <div class="flex bg-koyori-surface-2 rounded-full p-0.5 gap-0.5 shrink-0">
          <button
            ...
            :key="dr"
            class="px-3 py-1 rounded-full text-[12px] text-koyori-ink-3 bg-transparent border-none cursor-pointer font-[inherit] transition-all duration-150"
            :class="activeDateRange === dr ? '!bg-white !text-koyori-ink !font-semibold shadow-koyori-sm' : ''"
            @click="activeDateRange = dr"
          >{{ dr }}</button>
        </div>
        <!-- Min score -->
        <div class="flex items-center gap-2 ml-auto">
          <span class="text-[12px] text-koyori-ink-3 whitespace-nowrap">Min score: {{ minScore }}%</span>
          <input
            type="range"
      ...
      <template v-else-if="devState === 'Error'">
        <div class="bg-red-50 border border-red-200 rounded-xl p-5 text-center">
          ...
          <div class="text-[14px] font-semibold text-red-800 mb-1.5">Search failed</div>
          <div class="text-[13px] text-red-700">Could not reach search backend. Please try again.</div>
        </div>
      </template>
      ...
      <template v-else-if="devState === 'Default' && !searchQuery.trim()">
        <div class="text-center py-[60px] px-5 text-koyori-ink-3">
          ...
          <div class="text-[16px] font-semibold text-koyori-ink-2 mb-1.5">What are you looking for?</div>
          <div class="text-[13px] mb-5">Search across every transcript in your library</div>
      ...
      <!-- Empty results -->
      <template v-else-if="devState === 'Empty' || (searchQuery.trim() && filteredResults.length === 0)">
        <div class="text-center py-[60px] px-5 text-koyori-ink-3">
          <i class="pi pi-inbox text-[40px] block mb-3 text-koyori-ink-4"></i>
          <div class="text-[16px] font-semibold text-koyori-ink-2 mb-1.5">No results found</div>
          <div class="text-[13px]">Try different keywords or lower the minimum score</div>
        </div>
      </template>

      <!-- Results -->
      <template v-else>
        <div class="text-[12px] text-koyori-ink-3 mb-3">{{ filteredResults.length }} results</div>
        <div class="flex flex-col gap-3">
          <div
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start gap-2 mb-1.5 flex-wrap">
                <span class="text-[14px] font-semibold text-koyori-ink max-w-[400px] truncate">{{ result.stream }}</span>
                <span class="text-[12px] text-koyori-ink-4">{{ result.date }}</span>
                <span class="font-[JetBrains_Mono] text-[12px] text-koyori-pink-500 font-semibold bg-koyori-pink-50 px-1.5 py-px rounded-[5px]">{{ result.t }}</span>
                <span class="font-[JetBrains_Mono] text-[11px] text-koyori-ink-4">+{{ result.duration }}</span>
              </div>
              <p class="text-[13.5px] text-koyori-ink-2 m-0 mb-2 leading-relaxed">{{ result.snippet }}</p>
              <div class="flex items-center gap-2">
                <span class="text-[11px] text-koyori-ink-4 italic flex-1 min-w-0 truncate">{{ result.why }}</span>
                <!-- Score bar -->
                <div class="flex items-center gap-1.5 shrink-0">
                  <div class="w-[60px] h-1 bg-koyori-surface-3 rounded-full overflow-hidden">
                    <div
                      class="h-full bg-gradient-to-r from-koyori-pink-400 to-koyori-pink-500 rounded-full"
                      :style="{ width: `${result.score * 100}%` }"
                    ></div>
                  </div>
                  <span class="text-[12px] font-semibold text-koyori-ink-2">{{ (result.score * 100).toFixed(0) }}%</span>
                </div>
              </div>
            </div>
          </div>
...
import { DUMMY_SEARCH_RESULTS, useToneGradient } from '~/composables/koyori-data'
...
const suggestions = ['Lui-nee laughing', 'singing practice routine', 'CRISPR paper', 'Minecraft flask', 'outfit reveal']
