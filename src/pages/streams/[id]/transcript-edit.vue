<template>
  <div class="flex flex-col min-h-full">
    ...
    <div class="flex items-center justify-between px-8 py-4 border-b border-koyori-border bg-white sticky top-0 z-10 shrink-0">
      <div>
        <div class="flex items-center gap-1.5 text-[12px] text-koyori-ink-3 mb-1">
          ...
          <NuxtLink :to="`/streams/${streamId}`" class="text-koyori-ink-3 hover:text-koyori-ink no-underline max-w-[160px] overflow-hidden text-ellipsis whitespace-nowrap inline-block">{{ stream?.title }}</NuxtLink>
          <i class="pi pi-chevron-right text-[10px]"></i>
          <NuxtLink :to="`/streams/${streamId}/transcript`" class="text-koyori-ink-3 hover:text-koyori-ink no-underline">Transcript</NuxtLink>
          <i class="pi pi-chevron-right text-[10px]"></i>
          <span class="text-koyori-ink">Edit</span>
        </div>
        <h2 class="text-[18px] font-bold text-koyori-ink m-0 leading-tight">Edit transcript</h2>
        <p v-if="editedCount > 0" class="text-[13px] m-0 mt-0.5 text-koyori-pink-600">
          {{ editedCount }} unsaved change{{ editedCount !== 1 ? "s" : "" }}
        </p>
        <p v-else class="text-[13px] text-koyori-ink-3 m-0 mt-0.5">No unsaved changes</p>
      </div>
    ...
    <div class="flex-1 px-8 py-6 max-w-[900px] w-full">
      <!-- Info banner -->
      <div class="flex items-start gap-2.5 bg-blue-50 border border-blue-200 rounded-[10px] p-3 mb-5">
        <i class="pi pi-info-circle text-blue-600 text-[14px] pt-px shrink-0"></i>
        <span class="text-[13px] text-blue-800 leading-relaxed">Edits are flagged as human-corrected so the AI re-summary trusts them. Audio is never modified.</span>
      </div>
      ...
      <div class="flex flex-col gap-1">
        <div
          ...
          :class="editingIndex === i ? 'border-[1.5px] border-koyori-pink-400 shadow-[0_0_0_3px_rgba(244,139,169,0.15)]' : 'border border-koyori-border'"
        >
          <div class="flex items-center gap-2.5">
            <!-- Drag handle -->
            <i class="pi pi-bars text-[12px] text-koyori-ink-4 cursor-grab shrink-0"></i>
            <!-- Timestamp -->
            <span class="font-[JetBrains_Mono] text-[12px] bg-koyori-surface-2 text-koyori-pink-500 px-[7px] py-0.5 rounded-[5px] shrink-0">{{ line.t }}</span>
            <!-- Speaker dropdown -->
            <select
            ...
            <div class="flex-1 min-w-0">
              ...
              >
                <span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap">{{ line.text }}</span>
                <i
          ...
          <div v-if="editingIndex === i" class="mt-2">
            ...
            <div class="flex items-center gap-2 mt-1.5">
              <span class="text-[11px] text-koyori-ink-4">{{ line.text.length }} chars</span>
              <span class="text-[11px] text-koyori-ink-4">·</span>
              <span class="text-[11px] text-koyori-ink-4">confidence: 0.91</span>
              <div class="ml-auto flex gap-1.5">
                <button class="text-[11px] px-2 py-0.5 border border-koyori-border rounded-[5px] bg-white cursor-pointer text-koyori-ink-2 font-[inherit]">
                  Split
                </button>
                <button class="text-[11px] px-2 py-0.5 border border-koyori-border rounded-[5px] bg-white cursor-pointer text-koyori-ink-2 font-[inherit]">
                  Merge ↓
