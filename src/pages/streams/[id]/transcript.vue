    ...
    >
      <div>
        ...
        >
          ...
          <NuxtLink
            :to="`/streams/${streamId}`"
            class="text-koyori-ink-3 hover:text-koyori-ink no-underline max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap inline-block"
    ...
    <div class="flex-1 px-8 py-6 max-w-[900px] w-full">
      ...
      <div class="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div class="flex bg-koyori-surface-2 rounded-full p-0.5 gap-0.5">
          ...
          >
            {{ spk.label }}
            <span class="opacity-60 text-[11px]">{{ spk.count }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-px">
        ...
        >
          <!-- Timestamp -->
          <span
            class="font-[JetBrains_Mono] text-[12px] text-koyori-pink-500 shrink-0 w-[70px] pt-0.5 cursor-pointer"
            >{{ line.t }}</span
          >
          <!-- Speaker chip -->
          <span
...
const koyoriCount = computed(
  () => DUMMY_TRANSCRIPT.filter((l) => l.speaker === "koyori").length,
);
const unknownCount = computed(
  () => DUMMY_TRANSCRIPT.filter((l) => l.speaker === "unknown").length,
);
