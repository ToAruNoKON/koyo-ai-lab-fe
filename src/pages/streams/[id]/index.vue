    ...
    >
      <div>
        ...
        >
          ...
          <span
            class="text-koyori-ink max-w-[300px] overflow-hidden text-ellipsis whitespace-nowrap"
            >{{ stream?.title }}</span
          >
        </div>
        <h2
        ...
        <p
          class="text-[13px] text-koyori-ink-3 m-0 mt-0.5 font-[JetBrains_Mono]"
        >
          {{ stream?.duration }}
        </p>
      </div>
      <div class="flex gap-2 items-center relative">
        ...
        <div
          v-if="showMoreMenu"
          class="absolute right-0 top-full mt-1 bg-white border border-koyori-border rounded-[10px] shadow-koyori-lg p-1.5 min-w-[220px] z-50"
        >
          <template
          ...
          >
            <div
              v-if="item.divider"
              class="border-t border-koyori-border my-1"
            ></div>
            <button
              v-else
    ...
    <div class="flex-1 px-8 py-6 max-w-[1100px] w-full">
      ...
      <template v-if="isLoading || devState === 'Loading'">
        <div class="flex gap-6 mb-7">
          <div
            class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] w-[380px] h-[214px] rounded-xl shrink-0"
          ></div>
          <div class="flex-1 flex flex-col gap-3 pt-1">
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-7 w-[80%] rounded-[6px]"
            ></div>
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-4 w-[50%] rounded-[6px]"
            ></div>
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-20 w-full rounded-lg"
            ></div>
          </div>
        </div>
        <div class="flex gap-2.5 mb-5">
          <div
            class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-5 w-[90px] rounded-full"
          ></div>
          <div
            class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-5 w-[90px] rounded-full"
          ></div>
        ...
        <div
          class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-[200px] rounded-xl"
        ></div>
      </template>
        ...
        <div class="bg-red-50 border border-red-200 rounded-xl p-5 mb-5">
          <div class="flex items-center gap-2 mb-2">
            ...
            <span class="font-semibold text-red-800"
              >Failed to load stream</span
            >
            <span
              v-if="streamErrorStatus"
              class="text-[12px] text-red-600 font-[JetBrains_Mono]"
              >({{ streamErrorStatus }})</span
            >
          </div>
          <pre
            v-if="streamErrorData?.detail"
            class="bg-red-100 rounded-lg p-3 text-[12px] text-red-900 m-0 font-[JetBrains_Mono] overflow-x-auto"
            >{{ JSON.stringify(streamErrorData.detail, null, 2) }}</pre
          >
      ...
      <template
        v-else-if="
          devState === 'Not transcribed' ||
          (streamData && !stream?.transcribed && devState === 'Default')
        "
      >
        <!-- Hero -->
        <div class="flex gap-6 mb-7">
          <div
            ...
            :style="{
              background: stream?.thumbnail_url
                ...
                : useToneGradient(stream?.tone || 'pink'),
            }"
          >
            <div
              v-if="stream?.duration"
              class="absolute bottom-3 right-3 bg-black/55 text-white text-[12px] font-[JetBrains_Mono] px-2 py-0.5 rounded-[5px]"
            >
              {{ stream?.duration }}
            </div>
          </div>
          <div class="flex-1">
            <div class="mb-3.5">
              <span
                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11.5px] font-medium bg-amber-50 text-amber-700 border border-amber-200 mb-2.5"
              >
                <i class="pi pi-clock text-[11px]"></i> not transcribed
                yet<template v-if="stream?.transcription_status === 'pending'">
                  · queued</template
                >
              </span>
            </div>
            <div
              class="text-[15px] font-bold text-koyori-ink mb-1.5 max-w-full line-clamp-2 overflow-hidden"
            >
              {{ stream?.title }}
            </div>
            <div
              class="text-[13px] text-koyori-ink-3 mb-3 font-[JetBrains_Mono]"
            >
              {{ stream?.date
            ...
            <div class="flex gap-1.5 flex-wrap mb-4">
              <span
                ...
                :key="tag"
                class="text-[11px] px-2 py-0.5 rounded-[10px] bg-koyori-surface-2 text-koyori-ink-3"
                >#{{ tag }}</span
              >
            </div>
            <PButton
        ...
        <!-- Preview skeleton -->
        <div
          class="bg-white border border-koyori-border rounded-xl p-5 relative overflow-hidden"
        >
          <div class="text-[13px] font-semibold text-koyori-ink-3 mb-3.5">
            Preview (available after processing)
          </div>
          <div class="flex flex-col gap-2">
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-3.5 w-[95%] rounded"
            ></div>
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-3.5 w-[80%] rounded"
            ></div>
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-3.5 w-[88%] rounded"
            ></div>
            <div
              class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-3.5 w-[60%] rounded"
            ></div>
          </div>
          <!-- Gradient fade -->
          <div
            class="absolute bottom-0 left-0 right-0 h-[60px] bg-gradient-to-t from-white to-transparent"
          ></div>
        </div>
      </template>
      ...
      <template v-else-if="devState === 'Regenerating'">
        ...
        <!-- Progress card -->
        <div
          class="bg-white border border-koyori-border rounded-xl shadow-koyori-sm p-5 mb-4"
        >
          <div class="text-[14px] font-semibold text-koyori-ink mb-4">
            Re-processing…
          </div>
          <div class="flex flex-col gap-2.5">
            <div
              v-for="stage in regenStages"
              :key="stage.name"
            ...
            >
              ...
              <span
                ...
                :class="
                  stage.status === 'active'
                    ? 'text-koyori-ink font-semibold'
                    : stage.status === 'pending'
                      ? 'text-koyori-ink-4 font-normal'
                      : 'text-koyori-ink font-normal'
                "
              >
                {{ stage.name }}
                <span
                  v-if="stage.status === 'active'"
                  class="text-koyori-pink-500 text-[12px]"
                >
                  · 42%</span
                >
              </span>
        ...
        <div
          class="bg-gradient-to-r from-koyori-surface-2 via-koyori-surface-3 to-koyori-surface-2 bg-[size:200%_100%] animate-[shimmer_1.5s_ease-in-out_infinite] h-[180px] rounded-xl"
        ></div>
      </template>
      ...
      <template v-else>
        ...
        <div class="flex gap-6 mb-7">
          ...
          <div class="flex-1">
            <div class="mb-2.5">
              <span
                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11.5px] font-medium bg-koyori-emerald-50 text-koyori-emerald-700 border border-koyori-emerald-200 mb-2.5"
              >
        ...
        <div class="flex border-b border-koyori-border mb-5">
          <button
            ...
            :class="{
              '!text-koyori-pink-600 !border-koyori-pink-500':
                activeTab === tab.id,
            }"
        ...
        <div v-if="activeTab === 'summary'">
          <div class="flex gap-6">
            <div class="flex-1">
              <div class="text-[15px] font-semibold text-koyori-ink mb-2.5">
                TL;DR
              </div>
              <p class="text-[14px] text-koyori-ink-2 leading-relaxed mb-5">
                {{ stream?.summary }}
              </p>
              <div
              ...
              <ul class="p-0 m-0 list-none flex flex-col gap-1.5">
                <li
                  v-for="m in DUMMY_MOMENTS.filter(
                    (x) => x.type === 'highlight',
                  ).slice(0, 4)"
                  :key="m.t"
                ...
                >
                  <span
                    class="font-[JetBrains_Mono] text-[12px] text-koyori-pink-500 pt-px shrink-0"
                    >{{ m.t }}</span
                  >
                  <span>{{ m.label }}</span>
                </li>
            ...
            <div class="w-[200px] shrink-0 flex flex-col gap-2.5">
              <div
                class="bg-white border border-koyori-border rounded-xl shadow-koyori-sm p-3.5"
              >
                <div
                  class="text-[11px] text-koyori-ink-4 uppercase tracking-[0.04em] mb-1"
                >
                  Duration
                </div>
                <div
                  class="text-[18px] font-bold text-koyori-ink font-[JetBrains_Mono]"
                >
                  {{ stream?.duration }}
                </div>
              </div>
        ...
        <div v-else-if="activeTab === 'moments'" class="flex flex-col gap-2">
          ...
          >
            <span
              class="font-[JetBrains_Mono] text-[12.5px] text-koyori-pink-500 shrink-0 w-[70px]"
              >{{ moment.t }}</span
            >
            <span
            ...
            >
            <span
              class="flex-1 text-[13.5px] text-koyori-ink overflow-hidden text-ellipsis whitespace-nowrap max-w-full"
              >{{ moment.label }}</span
            >
            <!-- Score bar -->
            <div class="w-20 shrink-0">
              ...
              <div class="text-right text-[11px] text-koyori-ink-3 mt-0.5">
        ...
        <div v-else-if="activeTab === 'chapters'" class="flex flex-col gap-1.5">
          ...
          >
            <span
              class="text-[12px] text-koyori-ink-4 w-5 text-right shrink-0"
              >{{ i + 1 }}</span
            >
            <span
              class="font-[JetBrains_Mono] text-[12.5px] text-koyori-pink-500 shrink-0 w-[70px]"
            ...
            >
            <span
              class="flex-1 text-[13.5px] text-koyori-ink overflow-hidden text-ellipsis whitespace-nowrap"
              >{{ chapter.label }}</span
            >
            <span
              class="text-[12px] text-koyori-ink-4 shrink-0 font-[JetBrains_Mono]"
              >{{ chapter.dur }}</span
            >
          </div>
        </div>
...
const streamErrorData = computed(
  () => (streamError.value as { data?: unknown })?.data,
);

const stream = computed(() => {
  if (!streamData.value) return null;
  const s = streamData.value;
  return {
    id: s.id,
    title: s.title,
